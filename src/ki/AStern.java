...
public class AStern implements GraphSearch {
	...
	public Vertex[] getSolution() {
		...
		// changes the position of the goal vertex from the last to the second.
	...
	public void expandNode(Vertex currentNode) {
		...
		for (Vertex v : this.pSearchGraph.getVerticesArray()) {
			...
			if (currentNode.hasEdgeTo(v)) {
				...
				Float tentative_g = g(v, currentNode)
						+ (Float) currentNode.getAttribute("cost");

				if (open.contains(v)
						&& tentative_g >= (Float) v.getAttribute("cost"))
					continue;
	...
	/**
	 * Calculates the g function for the distance from the predecessor to the
	 * current selected vertex. It's uses the euklidian distance.
	 * 
	 * @param successor
	 *            vertex of the predecessor
	 * @param predecessor
	 *            vertex
	...
	public float g(Vertex successor, Vertex predecessor) {
		...
		return (float) Math.sqrt((Math.pow((xpredecessor - xv), 2) + Math.pow(
				(ypredecessor - yv), 2)));
	}

	/**
	 * Calculates a function for the distance from the the current selected
	 * vertex to the goal vertex. It's uses the euklidian distance.
	 * 
	 * @param v
	 *            vertex for calculating the distance to the goal
	 * @param predecessor
	 *            vertex
	...
	public float h(Vertex v) {
		...
		return (float) Math.sqrt((Math.pow((xv - xgoal), 2) + Math.pow(
				(yv - ygoal), 2)));
	}
...
/**
 * Helper class for the pointers. The is used for calculating the solution.
 * 
 * @author juerg
 * 
 */
